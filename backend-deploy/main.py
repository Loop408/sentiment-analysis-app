...
# Load model and vectorizer - all files in same directory for Vercel
...
def analyze_hashtag(request: HashtagRequest):
    try:
        ...
        # Normalize the search term: make lowercase and strip leading '#'
        hashtag_pattern = request.hashtag.lower().lstrip("#")
        filtered_tweets = combined_data[
            combined_data["sentence"].str.lower().str.contains(hashtag_pattern, na=False)
        ]
        ...
        sample_tweets = tweets_analysis[:10]
        word_cloud_data = []
        seen_words = set()
